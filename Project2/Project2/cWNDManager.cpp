...
LRESULT CALLBACK cWNDManager::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE:
	{
		...
		break;
	case WM_KEYDOWN:
		if (wParam == 'e' || wParam == 'E')	// do we want to use the controller instead of the keyboard
		{
			if (controller)
			{
				controller = false;
		...
		if (!controller)//are we using the controller if not
		{
		...
		break;
	case WM_KEYUP:// we keep moving till we let go of a key
	{
		if (wParam == VK_LEFT)
		{
			if (!firstPerson)
